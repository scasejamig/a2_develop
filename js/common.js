let common = {
	...
	user_create_window: (plot_id = 0, e) => {		
        // actions
        cancel_event(e);
        common.menu_popup_hide_all('all');
        // vars
	...
	create_user: (plot_id, multi_plots = false) => {
		if ( plot_id == 0 || plot_id == '' )
			return;
		// vars
		if ( !multi_plots )
		{
		...
		let location = {dpt: 'user', act: 'user_create'};
		// call
		request({location: location, data: data}, (result) => {
			common.modal_hide();
			html('table', result.html);
	...
	user_create: () => {		
		let plot_id = gv('plot_id');		
		common.create_user(plot_id);		
    },
	
	user_delete_window: (plot_id, e) => {
        // actions
        cancel_event(e);
        common.menu_popup_hide_all('all');
        ...
        // vars
        let data = {plot_id: plot_id};
        let location = {dpt: 'user', act: 'user_delete_window'};
        // call
        request({location: location, data: data}, (result) => {
            common.modal_show(400, result.html);
        });
